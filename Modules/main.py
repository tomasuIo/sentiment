...
from Modules.use_dalianligong_dic import _text_processing_
...
neg_corpus_root_path = os.getcwd() + '/res\corpus\\hotel\\neg\\'  #酒店评价负面情绪语料库目录
...
def __init_zhiwang_dic__():
    ...
    zhiwang_pos_sen_dic = Modules.LoadDictionary.load_dic(__path__, "pos_comment.txt", 1)  # 初始化正面情感词词典
    Modules.LoadDictionary.append_dic(zhiwang_pos_sen_dic, __path__, "pos_sentiment.txt", 1)
    zhiwang_neg_sen_dic = Modules.LoadDictionary.load_dic(__path__, "neg_sentiment.txt", -1)  # 初始化负面情感词词典
    Modules.LoadDictionary.append_dic(zhiwang_neg_sen_dic, __path__, "neg_comment.txt", -1)
...
def __init_tsinghua_dic__():
    ...
    tsinghua_pos_dic = Modules.LoadDictionary.load_dic(__path__,"tsinghua.positive.gb.txt",1)
    tsinghua_neg_dic = Modules.LoadDictionary.load_dic(__path__,"tsinghua.negative.gb.txt",-1)
...
def __init_ntusd_dic__():
	...
	ntusd_pos_dic = Modules.LoadDictionary.load_dic(__path__,"ntusd-positive.txt",1)
	ntusd_neg_dic = Modules.LoadDictionary.load_dic(__path__,"ntusd-negative.txt",1)
...
def __init_dllg_dic__():
    ...
    dllg_dic_meta_list = Modules.LoadDictionary.read_xlsx_file(__path__,"SenDic.xlsx")

#初始化情感词极值词典
def __init_extreme_dic__():
    __path__ = dic_root_path + "extreme_of_word\\"
    global word_extreme_dic
    word_extreme_dic = Modules.LoadDictionary.load_extreme_dic(__path__, "extreme.txt")
...
def __init_dic__(dictory_kind):
    global ext_dic
    __path__ = dic_root_path + "zhiwang\\"
    ext_dic = Modules.LoadDictionary.load_ext_dic(__path__, "extent_Lv_")  # 初始化程度副词词典
